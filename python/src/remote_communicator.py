...
class RemoteCommunicatorSSH(RemoteCommunicator):
    ...
    def __init__(self, wd_hosts_out=None, reuse_ssh=True):
        super(RemoteCommunicatorSSH, self).__init__(wd_hosts_out)
    ...
    def connect(self, host):
        ...
        if self.reuse_ssh:
            # next ssh con will use master conn.
            real_cmd = self.ssh_exe + ' -N ' + self.host
            self.ssh_master = subprocess.Popen(
                shlex.split(real_cmd), shell=False)
    ...
    def send_file(self, local_file, remote_file):
        ...
        if not os.path.exists(local_file):
            raise Exception("local_file: " + local_file + " not found on this "
                            "host")

        # check remote_file (do not overwrite)
        # fixme: overwrite should be a boolean parameter?
        # fixme: add check to paramiko too
        file_exists = self.launch(
            self.ls_exe + ' ' + remote_file, detached=False,
            quiet=True)
        if file_exists == 0:
            self.log("do not send already existing file: " + local_file)
            return
        ...
        # copy file perms if needed
        local_file_mod = str(oct(os.stat(local_file)[stat.ST_MODE] & 0777))
        # 0644 is set by > by default
        if (local_file_mod != '0644'):
            self.launch(
                self.chmod_exe + ' ' + local_file_mod + ' ' + remote_file)
...
class HandleSSH(object):
    ...
    def check_file_exists(self):
        ...
        real_cmd += ' ' + self.rc_ssh.ls_exe + ' ' + self.remote_file

        stdout = subprocess.PIPE
        stderr = subprocess.PIPE

        p = subprocess.Popen(
            shlex.split(real_cmd), stdout=stdout, stderr=stderr)
        p.communicate()
        ret = p.wait()
        if ret == 255:
            raise Exception('SSH error, check hostname (command :' + real_cmd +
    ...
    def read(self, size=-1):
        if self.mode != 'r':
            raise Exception(
                'handle open in wrong mode : ' + str(self.mode) + ')')
    ...
    def real_read(self, size=-1):
        ...
        if self.real_pos == 0:
            ...
            real_cmd += " '" + self.rc_ssh.tail_exe + \
                ' -c +' + str(self.real_pos + 1)
...
class RemoteCommunicatorParamiko(RemoteCommunicator):
    ...
    def __init__(self, wd_hosts_out=None):
        super(RemoteCommunicatorParamiko, self).__init__(wd_hosts_out)
